...
const StandardImageList = (props) => {
			...
			if (chainId === 4) {
				...
				if (tokenId) {
					if (props.stakeState === true) {
						...
						await nftContract.approve(CrocosFarmAddr, tokenId);
						await nftCon.wait();
	...
	return (
		<ImageList sx={{ width: 'auto', height: 450, padding: '50px' }} cols={4} style={{ display: "flex", justifyContent: "center" }}>
		...
		try {
			...
			if (chainId === 4) {
				...
				for (var i = 0; i < stakeOfOwner.length; i++) {
					...
					const nftMetaData = await axios.get(tokenURI);
					const nftTokenData = { img: nftMetaData.data.image, title: nftMetaData.data.name, tokenId: stakeOfOwner[i] }
					tokenData.push(nftTokenData);
				}
				setTokensOfOwner(tokenData);
				console.log(tokenData)
